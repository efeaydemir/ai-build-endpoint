// api/build.ts — v6: ASSET op + prompt'tan otomatik ID yakalama
export const config = { runtime: "edge" };

const MODEL = "llama-3.1-8b-instant";

const SYSTEM = `
Return STRICT JSON with a "program" for a Roblox builder.
Allowed ops:
- PLAZA {size,color}
...
- ASSET {assetId|url, pos:[x,y,z], yaw}
- LABEL {text}
Return ONLY:
{"label":"2-3 words","palette":["#RRGGBB","#RRGGBB","#RRGGBB"],"program":[ ... ]}
Keep program <= 24 steps. No prose.
`;
...
function extractAssetIdsFromPrompt(p:string): number[] {
  ...
  while ((m = rx.exec(p)) !== null) {
    const id = Number(m[1]); if (id) ids.add(id);
  }
  return Array.from(ids).slice(0, 12); // aşırıya kaçmasın
}
...
function expand(program:any[], palette:string[]) {
  ...
  // (diğer op genişleticileri kısalttım – önceki sürümdekiyle aynı: RING, DISC, SPIRAL, TOWER, PYRAMID, DOME, HOUSES, TREES, RIVER, BRIDGE)
  ...
  const addHouses=(rows:number,cols:number,minH:number,maxH:number)=>{ const sp=6, sx=-((cols-1)*sp)/2, sz=12; for(let r=0;r<rows;r++) for(let c=0;c<cols;c++){ const h=Math.round(clamp(minH,1,4)+(c%2)*(clamp(maxH,2,8)-clamp(minH,1,4))); const x=sx+c*sp, z=sz+r*sp; acts.push({type:"PLACE_BLOCK",block:"SmoothPlastic",pos:[x,1,z],size:[4,h,4],color:pal[0]}); acts.push({type:"PLACE_BLOCK",block:"Rock",pos:[x,h+1,z],size:[4,1,4],color:"#6B4F2A"}); if(acts.length>MAX_ACTIONS)break; } };
  ...
  for (const s of (program||[])) {
    ...
    switch (s.op) {
      ...
      case "LINE": addLine(s.from||[0,0], s.to||[0,0], clamp(s.width??2,1,8), clamp(s.height??1,1,8), s.block||"Slate", s.color); break;
      case "RING": addRing(clamp(s.r??8,2,40), clamp(s.height??1,0,20), clamp(s.step??3,1,20), s.block||"Neon", s.color); break;
...
export default async (req: Request) => {
  ...
  try {
    ...
    // 1) LLM: program üret (ASSET op'u da bilecek)
    ...
    if (r.ok) {
      ...
      try { plan = JSON.parse(j.choices?.[0]?.message?.content || "{}"); } catch {}
    }

    let program:any[] = Array.isArray(plan.program) ? plan.program.slice(0,24) : [];
    ...
    // 2) Prompt içindeki asset link/ID'lerini otomatik ekle
    const found = extractAssetIdsFromPrompt(prompt);
    if (found.length) {
      // basit grid dizilimi
      const baseY = 1, gap = 10;
    ...
    return new Response(JSON.stringify({ actions, reason:"v6 program+assets" }), { status:200, headers:{"Content-Type":"application/json"}});
  } catch (e:any) {
